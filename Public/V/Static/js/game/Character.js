...
Character.prototype.checkRoad = function (dir){
	...
	if(toy >= mapPass.length || tox >= mapPass[0].length)return false;
	//如果目的地为障碍，则不可移动
	if(mapPass[toy][tox] == 1)return false;
	return true;
};
/**
 * 设定人物坐标
...
Character.prototype.checkMap = function (dir){
	...
	switch (dir){
		case UP:
			...
			if(mapDownLayer.y >= 0)break;
			addMap(0,-1);
			mapmove = true;
			break;
		case LEFT:
			if(self.x + charaLayer.x > STEP)break;
			if(mapDownLayer.x >= 0)break;
			addMap(-1,0);
			mapmove = true;
			break;
		case RIGHT:
			if(self.x < 480 - 2*STEP)break;
			if(480 - mapDownLayer.x >= map[0].length*STEP)break;
			addMap(1,0);
			mapmove = true;
			break;
		case DOWN:
			if(self.y < 288 - 2*STEP)break;
			if(288 - mapDownLayer.y >= map.length*STEP)break;
			addMap(0,1);
			mapmove = true;
			break;
	}
