	...
	RPG.ItemList= [
		{index:0, pic:{x:12,y:0}, name:"神鞭", kind: 1, type: 1, addon:1.5, atkEff:"pSwipe", desc:"魔王的神鞭，从地底洞穴中得到，具有强大的威力", effect:function(ahero){ahero.changeWeopen(0);}},
		{index:1, pic:{x:3,y:10}, name:"肉干", kind: 2, type:1, desc:"肉干，食用可以补充少量体力", effect:function(ahero){ahero.addHp(200);}},
		{index:2, pic:{x:0,y:10}, name:"胡萝卜", kind: 2, type:2, desc:"营养丰富的蔬菜", effect:function(ahero){ahero.addMp(20);}},
		...
		{index:12, pic:{x:10,y:10}, name:"面包", kind: 2, type:1, desc:"面包，适合携带，食用可以补充体力", effect:function(ahero){ahero.addHp(500);}},
		{index:13, pic:{x:8,y:10}, name:"蓝莓", kind: 4, type:1, desc:"蓝色浆果，可在战场食用快速补充体力", effect:function(ahero){ahero.addHp(200);}},
	]
	// 全部技能
	RPG.SkillList= [
	...
	RPG.Item= {
		index: 0,
		num: 0,
		useTo: function(ahero){
			if (RPG.ItemList[this.index].effect){
				RPG.ItemList[this.index].effect(ahero);
			}
		},
	...
	// 获得浆果的通用程序
	RPG.getBerry= function(aId, aNum, aTalk) {
		var str1= "Berry"+ aId;
		if (!aNum) {aNum= 1;}
		if (!RPG.checkSwitch(str1)) {
			if (!RPG.checkSwitch("firstBerry")) {
				RPG.setSwitch("firstBerry");
			}
     		mainTeam.addItem(13, aNum, true);
			RPG.setSwitch(str1);
			if (aTalk) {
		     	RPG.startTalk(aTalk);
			}
